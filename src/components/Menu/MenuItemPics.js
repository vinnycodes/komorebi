// Appetizers Imports
import crabRangoonImg from '../../media/menu/apps/crab_rangoon.jpg';
import edamameImg from '../../media/menu/apps/edamame.jpg';
import calamariImg from '../../media/menu/apps/fried_calamari.jpg';
import gyozaImg from '../../media/menu/apps/gyoza.jpg';
import tempuraImg from '../../media/menu/apps/tempura.jpg';
import springRollImg from '../../media/menu/apps/spring_roll.jpg';

// Soups & Salads
import misoImg from '../../media/menu/soups&salads/miso_soup.jpg';
import wontonImg from '../../media/menu/soups&salads/wonton.jpg';
import phoImg from '../../media/menu/soups&salads/pho.jpg';
import ramenImg from '../../media/menu/soups&salads/ramen.jpg';
import houseSaladImg from '../../media/menu/soups&salads/asian_salad.jpg';
import seaweedSaladImg from '../../media/menu/soups&salads/seaweed_salad.jpg';

// Sushi
import sushi_1 from '../../media/menu/sushi/sushi-1.jpg';
import sushi_2 from '../../media/menu/sushi/sushi-2.jpg';
import sushi_3 from '../../media/menu/sushi/sushi-3.jpg';
import sushi_4 from '../../media/menu/sushi/sushi-4.jpg';
import sushi_5 from '../../media/menu/sushi/sushi-5.jpg';
import sushi_6 from '../../media/menu/sushi/sushi-6.jpg';
import sushi_7 from '../../media/menu/sushi/sushi-7.jpg';
import sushi_8 from '../../media/menu/sushi/sushi-8.jpg';
import sushi_9 from '../../media/menu/sushi/sushi-9.jpg';
import sushi_10 from '../../media/menu/sushi/sushi-10.jpg';
import sushi_11 from '../../media/menu/sushi/sushi-11.jpg';

// Specialty Rolls
import specialtyRoll_1_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-1.jpg';
import specialtyRoll_2_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-2.jpg';
import specialtyRoll_3_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-3.jpg';
import specialtyRoll_4_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-4.jpg';
import specialtyRoll_5_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-5.jpg';
import specialtyRoll_6_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-6.jpg';
import specialtyRoll_7_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-7.jpg';
import specialtyRoll_8_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-8.jpg';
import specialtyRoll_9_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-9.jpg';
import specialtyRoll_10_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-10.jpg';
import specialtyRoll_11_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-11.jpg';
import specialtyRoll_12_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-12.jpg';
import specialtyRoll_13_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-13.jpg';
import specialtyRoll_14_Imgs from '../../media/menu/specialtyRolls/specialtyRoll-14.jpg';

// Rice And Noodles
import riceAndNoodles_1 from '../../media/menu/rice&noodles/Pad See Ew.jpg';
import riceAndNoodles_2 from '../../media/menu/rice&noodles/House Fried Rice.jpg';
import riceAndNoodles_3 from '../../media/menu/rice&noodles/House Lo Mein.jpg';
import riceAndNoodles_4 from '../../media/menu/rice&noodles/Pad Thai.jpg';

export const menuPics = {
  // Appetizers
  crabRangoon: crabRangoonImg,
  edamame: edamameImg,
  calamari: calamariImg,
  gyoza: gyozaImg,
  tempura: tempuraImg,
  springRoll: springRollImg,

  // Soups & Salads
  miso: misoImg,
  wonton: wontonImg,
  pho: phoImg,
  ramen: ramenImg,
  houseSalad: houseSaladImg,
  seaweedSalad: seaweedSaladImg,

  // Sushi Rolls
  sushi_1: sushi_1,
  sushi_2: sushi_2,
  sushi_3: sushi_3,
  sushi_4: sushi_4,
  sushi_5: sushi_5,
  sushi_6: sushi_6,
  sushi_7: sushi_7,
  sushi_8: sushi_8,
  sushi_9: sushi_9,
  sushi_10: sushi_10,
  sushi_11: sushi_11,

  // Speciality Rolls
  specialtyRoll_1: specialtyRoll_1_Imgs,
  specialtyRoll_2: specialtyRoll_2_Imgs,
  specialtyRoll_3: specialtyRoll_3_Imgs,
  specialtyRoll_4: specialtyRoll_4_Imgs,
  specialtyRoll_5: specialtyRoll_5_Imgs,
  specialtyRoll_6: specialtyRoll_6_Imgs,
  specialtyRoll_7: specialtyRoll_7_Imgs,
  specialtyRoll_8: specialtyRoll_8_Imgs,
  specialtyRoll_9: specialtyRoll_9_Imgs,
  specialtyRoll_10: specialtyRoll_10_Imgs,
  specialtyRoll_11: specialtyRoll_11_Imgs,
  specialtyRoll_12: specialtyRoll_12_Imgs,
  specialtyRoll_13: specialtyRoll_13_Imgs,
  specialtyRoll_14: specialtyRoll_14_Imgs,

  // Rice & Noodles
  riceAndNoodles_1: riceAndNoodles_1,
  riceAndNoodles_2: riceAndNoodles_2,
  riceAndNoodles_3: riceAndNoodles_3,
  riceAndNoodles_4: riceAndNoodles_4,
};

export default menuPics;
